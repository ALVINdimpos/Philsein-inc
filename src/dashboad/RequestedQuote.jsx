...
export default function RequestedQuote() {
  ...
  useEffect(() => {
    ...
    axios
      .get('https://kagaba.live/QuoteRequest')
      .then((response) => {
        // Handle the response data and update the state
        setRequestedQuote(response.data)
      })
      .catch((error) => {
  ...
  const handleDelete = (id) => {
    axios
      .delete(`https://kagaba.live/QuoteRequest/${id}`)
      .then((response) => {
  ...
  return (
    <Layout>
      <div className="flex justify-between items-center border-b border-gray-300">
        <h1 className="text-2xl font-semibold pt-2 pb-6">Requested Quotes</h1>
      </div>
      {/* STATISTICS */}
        ...
        <div className="bg-white shadow rounded-sm flex justify-between items-center py-3.5 px-3.5">
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-2">
              <h1 className="text-xl font-semibold">{requestedQuote.length}</h1>
            </div>
          </div>
          <svg
          ...
          <table className="min-w-max w-full table-auto">
            <thead>
              <tr className="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                ...
                <th className="py-3 px-6 text-left">Packaging Type</th>
