...
export default function quapps() {
    ...
    useEffect(() => {
        const getApps = async () => {
            ...
            const res = await (await fetch("http://localhost:3000/apps", config)).json();
            setApps(_ => res.data);
    ...
    const DeleteApp = async (deleteApp) => {
        ...
        const res = await (await fetch(`http://localhost:3000/apps/${deleteApp}`, {
            "method": "DELETE",
            ...config
        })).json();
        if (res.status === 200) {
                    ...
                    <tbody className="text-gray-600 text-sm">
                        ...
                        apps.map((app, idx) => (
                            <tr className="border-b border-gray-200 even:bg-gray-100 hover:bg-gray-100" key={idx}>
                                ...
                                <td className="py-3 px-6 text-center flex flex-col gap-2">
                                    {app.resume.map((link, idx) => (
                                        <a href={link} className='text-ellipsis' key={idx}>
                                            <span className="bg-purple-200 text-ellipsis text-purple-600 py-1 px-3 rounded-md text-xs">{link}</span>
                                        </a>
                                    ))}
                                </td>
                                <td className="py-3 px-6 text-center">
