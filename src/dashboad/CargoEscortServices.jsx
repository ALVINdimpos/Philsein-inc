...
export default function CargoEscortServices() {
    const [cargoEscortServices, setCargoEscortServices] = useState([]);
    useEffect(() => {
        // Fetch the data using axios GET request
        axios.get('https://kagaba.live/cargoEscortService')
            .then((response) => {
                // Handle the response data and update the state
                setCargoEscortServices(response.data)
            })
            .catch((error) => {
    ...
    const handleDelete = (id) => {
        ...
        axios.delete(`https://kagaba.live/cargoEscortService/${id}`)
            .then((response) => {
    ...
    return (
        <Layout>
            <div className="flex justify-between items-center border-b border-gray-300">
                <h1 className="text-2xl font-semibold pt-2 pb-6">CARGO ESCORT SERVICES</h1>
            </div>
            {/* STATISTICS */}
