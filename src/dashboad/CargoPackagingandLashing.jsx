...
export default function CargoPackagingandLashing() {
  const [cargoPackagingandLashing, setCargoPackagingandLashing] = useState([]);
  useEffect(() => {
    // Fetch the data using axios GET request
    axios.get('https://kagaba.live/cargopackagingandlashing')
      .then((response) => {
        // Handle the response data and update the state
        setCargoPackagingandLashing(response.data.records)
      })
      .catch((error) => {
  ...
  const handleDelete = (id) => {
    ...
    axios.delete(`https://kagaba.live/cargopackagingandlashing/${id}`)
      .then((response) => {
  ...
  return (
    <Layout>
      <div className="flex justify-between items-center border-b border-gray-300">
        <h1 className="text-2xl font-semibold pt-2 pb-6">CARGO PACKAGING AND LASHING</h1>
      </div>
      {/* STATISTICS */}
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-2">
              <h1 className="text-xl font-semibold">
                {cargoPackagingandLashing.length}
              </h1>
            </div>
          </div>
        ...
        <div className="bg-white shadow rounded-sm my-2.5 overflow-x-auto">
          <table className="min-w-max w-full table-auto">
            ...
            <tbody className="text-gray-600 text-sm">
              {cargoPackagingandLashing.map((query) => (
