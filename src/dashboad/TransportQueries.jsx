...
export default function TransportQueries() {
  ...
  useEffect(() => {
    ...
    axios
      .get('https://kagaba.live/querry')
      .then((response) => {
        // Handle the response data and update the state
        setQueries(response.data.querries)
      })
      .catch((error) => {
  ...
  const handleDelete = (id) => {
    axios
      .delete(`https://kagaba.live/querry/${id}`)
      .then((response) => {
  ...
  return (
    <Layout>
      <div className="flex justify-between items-center border-b border-gray-300">
        <h1 className="text-2xl font-semibold pt-2 pb-6">Queries</h1>
      </div>
      {/* STATISTICS */}
        ...
        <div className="bg-white shadow rounded-sm flex justify-between items-center py-3.5 px-3.5">
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-2">
              <h1 className="text-xl font-semibold">{querries.length}</h1>
            </div>
          </div>
          <svg
            ...
            <tbody className="text-gray-600 text-sm">
              {querries.map((query) => (
                ...
                >
                  ...
                  <td className="py-3 px-6 text-center">{query.phone}</td>
                  <td className="py-3 px-6 text-center">{query.subject}</td>
                  <td className="py-3 px-6 text-center">{query.message}</td>
