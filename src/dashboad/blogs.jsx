...
export default function blogs() {
  ...
  useEffect(() => {
      const getApps = async () => {
          ...
          const res = await (await fetch("http://localhost:3000/blogs/all", config)).json();
          setBlogs(_ => res.data);
  ...
  const DeleteBlog = async (deleteBlog) => {
      ...
      const res = await (await fetch(`http://localhost:3000/blogs/${deleteBlog}`, {
          "method": "DELETE",
          ...config
      })).json();
      if (res.status === 200) {
  ...
  const publishBlog = async (publishBlog) => {
      ...
      const res = await (await fetch(`http://localhost:3000/blogs/${publishBlog}`, {
          "method": "PATCH",
          ...config
      })).json();
      if (res.status === 200) {
  ...
  return (
    <Layout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 py-6">
        <div className="bg-white shadow rounded-sm flex justify-between items-center py-3.5 px-3.5">
            <div className="space-y-2">
                ...
                <div className="flex items-center space-x-2">
                    <h1 className="text-xl font-semibold">{published + unpublished}</h1>
                </div>
            </div>
        ...
        <div className="bg-white shadow rounded-sm flex justify-between items-center py-3.5 px-3.5">
            <div className="space-y-2">
                <p className="text-xs text-gray-400 uppercase">Total Published Blogs</p>
                <div className="flex items-center space-x-2">
        ...
        <div className="bg-white shadow rounded-sm flex justify-between items-center py-3.5 px-3.5">
            <div className="space-y-2">
                <p className="text-xs text-gray-400 uppercase">Total Unpublished Blogs</p>
                <div className="flex items-center space-x-2">
            ...
            <div className="blog flex" key={idx}>
              ...
              <div className="shadow-lg flex-1 max-[768px]:rounded-lg min-[769px]:rounded-r-lg p-2 flex flex-col gap-3 border">
                ...
                <div className="actions flex w-[80%] self-center gap-3">
                  <button type="button" className="publish w-[30%] bg-blue-500 rounded-md p-2 text-white" id={blog._id} onClick={handlePublish}>Publish</button>
                  <button type="button" className="delete w-[30%] bg-red-500 rounded-md p-2 text-white" id={blog._id} onClick={handleDelete}>Delete</button>
