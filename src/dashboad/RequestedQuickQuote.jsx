...
export default function RequestedQuickQuote() {
    const [requestedQuickQuote, setRequestedQuickQuote] = useState([]);
    console.log(requestedQuickQuote)
    useEffect(() => {
        // Fetch the data using axios GET request
        axios.get('https://kagaba.live/requestQuickQuote')
            .then((response) => {
                // Handle the response data and update the state
                setRequestedQuickQuote(response.data)
            })
            .catch((error) => {
    ...
    const handleDelete = (id) => {
        ...
        axios.delete(`https://kagaba.live/requestQuickQuote/${id}`)
            .then((response) => {
