...
export default function referrals() {
    ...
    useEffect(() => {
        const getApps = async () => {
            ...
            const res = await (await fetch("https://cautious-erin-pig.cyclic.app/referrals", config)).json();
            setApps(_ => res.data);
    ...
    const DeleteApp = async (deleteApp) => {
        ...
        const res = await (await fetch(`https://cautious-erin-pig.cyclic.app/referrals/${deleteApp}`, {
            "method": "DELETE",
            ...config
        })).json();
        if (res.status === 200) {
    ...
    return (
        <Layout>
            ...
            <h1 className="text-2xl font-semibold pt-2 pb-6">Refferals</h1>
            </div>
            {/* STATISTICS */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 py-6">
                ...
                <div className="bg-white shadow rounded-sm flex justify-between items-center py-3.5 px-3.5">
                    <div className="space-y-2">
                        <p className="text-xs text-gray-400 uppercase">Total Read referrals</p>
                        <div className="flex items-center space-x-2">
                ...
                <div className="bg-white shadow rounded-sm flex justify-between items-center py-3.5 px-3.5">
                    <div className="space-y-2">
                        <p className="text-xs text-gray-400 uppercase">Total Unread referrals</p>
                        <div className="flex items-center space-x-2">
                ...
                <table className="min-w-max w-full table-auto">
                    <thead>
                        <tr className="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                            <th className="py-3 px-6 text-center bg-gray-500 text-white">Referer:</th>
                            <th className="py-3 px-6 text-left">Name</th>
                            <th className="py-3 px-6 text-left">Email</th>
                            <th className="py-3 px-6 text-center">Phone</th>
                            <th className="py-3 px-6 text-center bg-gray-500 text-white">Referee:</th>
                            <th className="py-3 px-6 text-left">Name</th>
                            <th className="py-3 px-6 text-left">Email</th>
                            <th className="py-3 px-6 text-center">Phone</th>
                            <th className="py-3 px-6 text-center">Position</th>
                            <th className="py-3 px-6 text-center">Relationship</th>
                    ...
                    <tbody className="text-gray-600 text-sm">
                        ...
                        apps.map((app, idx) => (
                            <tr className="border-b border-gray-200 even:bg-gray-100 hover:bg-gray-100" key={idx}>
                                <td className="py-3 px-6 text-left bg-gray-500 text-white whitespace-nowrap">
                                    Referer
                                </td>
                                <td className="py-3 px-6 text-left">
                                    {app.referer.first_name} {app.referer.last_name}
                                </td>
                                <td className="py-3 px-6 text-left">
                                    {app.referer.email}
                                </td>
                                ...
                                <td className="py-3 px-6 text-center text-white bg-gray-500">
                                    Referee
                                </td>
                                <td className="py-3 px-6 text-center">
                                    {app.referee.first_name} {app.referee.last_name}
                                </td>
                                <td className="py-3 px-6 text-center">
                                    {app.referee.email}
                                </td>
                                ...
                                <td className="py-3 px-6 text-center">
                                    {app.referee.position}
                                </td>
                                <td className="py-3 px-6 text-center">
                                    {app.referee.relationship_with_referer}
                                </td>
                                <td className="py-3 px-6 text-center">
                        ...
                        <tr className=" border-b border-gray-200 hover:bg-gray-100">
                            <td className='h-10 flex items-center justify-center' colSpan="7"> There are no Available Applications</td>
                        </tr>
                        }
                    </tbody>
                </table>
