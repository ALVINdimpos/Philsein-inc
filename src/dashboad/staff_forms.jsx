...
export default function staff_forms() {
  ...
  useEffect(() => {
    const getApps = async () => {
      ...
      const res = await (
        await fetch('https://cautious-erin-pig.cyclic.app/staff', config)
      ).json()
      setApps((_) => res.data)
  ...
  const DeleteApp = async (deleteApp) => {
    ...
    const res = await (
      await fetch(`https://cautious-erin-pig.cyclic.app/staff/${deleteApp}`, {
        method: 'DELETE',
        ...config,
      })
    ).json()
              ...
              {apps.length ? (
                apps.map((app, idx) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-6 text-center">{app.city}</td>
                    <td className="py-3 px-6 text-center">{app.province}</td>
                    <td className="py-3 px-6 text-center">
                      <span
                        className={`${
                ...
                <tr className=" border-b border-gray-200 hover:bg-gray-100">
                  ...
                  >
                    {' '}
                    There are no Available Staff Applications
                  </td>
                </tr>
              )}
            </tbody>
