...
export default function staff_forms() {
    ...
    useEffect(() => {
        const getApps = async () => {
            ...
            const res = await (await fetch("https://cautious-erin-pig.cyclic.app/staff", config)).json();
            setApps(_ => res.data);
    ...
    const DeleteApp = async (deleteApp) => {
        ...
        const res = await (await fetch(`https://cautious-erin-pig.cyclic.app/staff/${deleteApp}`, {
            "method": "DELETE",
            ...config
        })).json();
        if (res.status === 200) {
